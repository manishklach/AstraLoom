...
export function northTransitChart(natal,transit){
  const asc=natal.cusps[0].signIndex;
  const cells=centres.map(([x,y],i)=>{
    const sign=(asc+i)%12,planets=transit.planets.filter(p=>p.signIndex===sign),compact=[1,2,4,5,7,8,10,11].includes(i);
    const labels=planets.map(p=>`${p.name.slice(0,2)}${p.retrograde?'ᴿ':''}`),rows=[];const chunk=Math.max(1,Math.ceil(labels.length/3));
    for(let j=0;j<labels.length;j+=chunk)rows.push(labels.slice(j,j+chunk).join(' '));
    const labelY=y-(rows.length?12:0),font=compact?12:14;
    return `<g><title>Transit house ${i+1}: ${SIGNS[sign]}. ${planets.map(p=>escape(p.name)).join(', ')||'No transits'}.</title><text x="${x}" y="${labelY}" class="north-sign">${sign+1}${i===0?' · Asc':''}</text>${rows.map((p,j)=>`<text x="${x}" y="${labelY+16+j*15}" class="north-planet north-transit-planet" font-size="${font}">${p}</text>`).join('')}</g>`;
  }).join('');
  return `<div class="north-wrap transit-north-wrap"><svg class="north-chart" viewBox="-3 -3 406 406" role="img" aria-label="Current transit North Indian chart. Houses use the natal ${SIGNS[asc]} ascendant, sign ${asc+1}."><rect x="0" y="0" width="400" height="400" class="north-paper"/><path d="M0 0L400 400M400 0L0 400M200 0L400 200L200 400L0 200Z" class="north-lines"/>${cells}</svg></div><p class="hint chart-key">Natal ascendant: ${SIGNS[asc]} (${asc+1}) in the upper central diamond. Transit planets are placed by their current sidereal sign; ᴿ marks retrograde motion.</p>`;
}