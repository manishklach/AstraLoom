...
test('native Python reference verifies every placement, nakshatra and sub chain',async()=>{const native=JSON.parse(await readFile('tests/fixtures/native-reference.json','utf8')),c=calculate(swe,input);c.cusps.forEach((p,i)=>assert.ok(Math.abs(p.longitude-native.cusps[i])<1e-8));c.planets.forEach(p=>{const reference=native.planets[p.name];assert.ok(Math.abs(p.longitude-reference.longitude)<1e-8,p.name);assert.equal(p.house,reference.house,`${p.name} house`);assert.deepEqual([p.star,p.sub,p.subsub],reference.lords,p.name);});assert.ok(Math.abs(c.seed.start-native.dasha_start_ms)<1);});
test('all cusp chains, true Rahu and five current dasha boundaries match native reference',async()=>{const n=JSON.parse(await readFile('tests/fixtures/native-reference.json','utf8')),c=calculate(swe,input);c.cusps.forEach((p,i)=>assert.deepEqual([p.star,p.sub,p.subsub],n.cusp_lords[i]));assert.ok(Math.abs(calculate(swe,{...input,node:'true'}).planets[7].longitude-n.planets.TrueRahu.longitude)<1e-8);const actual=chainAt(c.seed,Date.parse('2026-09-13T12:00:00Z')).chain;assert.equal(actual.length,5);actual.forEach((p,i)=>{assert.equal(p.lord,n.referenceChain[i].lord);assert.ok(Math.abs(p.start-n.referenceChain[i].start)<1);assert.ok(Math.abs(p.end-n.referenceChain[i].end)<1);});});
