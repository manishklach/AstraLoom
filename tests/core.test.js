...
test('all nested periods through Pran cover parents without gaps; starts rotate with lord',()=>{for(const md of mahadashas(seedDasha(0,348.49))){let list=[md];for(let depth=0;depth<4;depth++){const next=[];for(const p of list){const c=children(p);assert.equal(c[0].lord,p.lord);assert.equal(c[0].start,p.start);assert.equal(c.at(-1).end,p.end);c.forEach((x,i)=>{assert.ok(x.end>x.start);if(i)assert.equal(x.start,c[i-1].end);});next.push(...c);}list=next;}}});
